...
def expansion(adjacencymatrixchecked, logger = None):
    ...
    if isinstance(adjacencymatrixchecked, SafeCSR):
        ...
        adj_matrix = load_adjacency(adjacencymatrixchecked)
　　adj_matrix_original = adj_matrix.copy()
    # The original was adj_matrix = adjacencymatrixchecked.copy()
    expanded_adj_matrix = adj_matrix @ adj_matrix
    # The original was expanded_adj_matrix = adjacencymatrixchecked @ adj_matrix
...
　　adj_matrix_original = adj_matrix.copy()
    ...
    # The original was hd = np.square(adjacencymatrixchecked.data)
    adj_matrix.data = hd
...
def mclprocess(adjacencymatrixchecked, stepnum = 20, logger = None):
    if isinstance(adjacencymatrixchecked, SafeCSR):
        ...
        adj_matrix = load_adjacency(adjacencymatrixchecked)
    # The original was A = load_adjacency(adjacencymatrixchecked)
    log = resolve_logger(logger, "mcl")
    steps = 0
...
def mclus_analysis(mmoriginadj_, clusmemlist_, clussizelist_, corecluscandlist_, defaultcorenum = 0, logger = None):
    log = resolve_logger(logger, "mcl")
    ...
    if not isinstance(mmoriginadj_, coo_matrix):
        mmoriginadj_ = mmoriginadj_.tocoo()
...
def branching_rmcl(dic_mclresult, originadj, defaultcorenum=0, threspruning=1.0, reverse_process = False, logger = None):
    log = resolve_logger(logger, "mcl")
    mmoriginadj, clusmemlist, clussizelist, corecluscandlist = coreclusQ(dic_mclresult, originadj, logger=logger)
    _, coreclusternumber, _, _, _, deginfo, clusmemdeginfo, max_indices, allrepresentnodeslist,coreclustermember,values, _, _ = mclus_analysis(mmoriginadj, clusmemlist, clussizelist, corecluscandlist, defaultcorenum = 0, logger=logger)
...
def sr_mcl(dic_mclresult, originadj, defaultcorenum=0, coreinfoonly = True, logger = None):
    log = resolve_logger(logger, "mcl")
    mmoriginadj, clusmemlist, clussizelist, corecluscandlist = coreclusQ(dic_mclresult, originadj, logger=logger)    
    coreclushub, coreclusternumber, coreclus_values, coremapping, reverse_coremapping, _, _, _, _, _, _, _, _ = mclus_analysis(mmoriginadj, clusmemlist, clussizelist, corecluscandlist, defaultcorenum = 0)
...
mclus_anaysis = mclus_analysis  # backward compatibility
